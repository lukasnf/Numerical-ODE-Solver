...
class Solver:
     ...
     def get_function(self, func):
          func = func.lower()
          if not "y'" in func:
               f = self.aeval(f"lambda x,y: {func}")
               return f
          else:
               func = func.replace("y'","dy")
               f = self.aeval(f"lambda x,y,dy: {func}")
               return f

     def euler_2order(self,f,x0,y0,dy0,n,bound):
          if n <= 0:
     ...
     def euler_1order(self,f,x0,y0,n,bound):
          if n <= 0:
               raise ValueError("n must be greater than zero.")
          elif x0 > bound:
     ...
     def rk4_1order(self,f,x0,y0,n,bound):
          if n <= 0:
               raise ValueError("n must be greater than zero.")
          elif x0 > bound:
               raise ValueError("initial condition must be < than the inpunt bound!")
     ...
     def get_value(self,data,val,dec):
          x_vals = data[:,0]
          y_vals = data[:,1]
          function = interp1d(x_vals,y_vals,kind="cubic")
